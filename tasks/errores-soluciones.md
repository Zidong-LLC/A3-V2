...
**Progreso 2026-08-21 (Etapa 2 COMPLETADA):** con Fase 3 ya cerrada, se retomó la migración señal-primero. Convertidos al molde C1/C2/C3: el bloque de la reoferta de estables (_stable_confirm_pending), las correcciones en fase CONFIRMACIÓN (cambio de cliente + campo) y la oferta de análisis extra (con `signal` en la firma del carril). PRE_LLM_RETURNS_BASELINE 44→35. Antes: `tokenize` normaliza tildes (Etapa N) con invariante propio. Tests: test_etapa2_senal_confirmacion.py (mecánica con señal fingida, L51). Pendiente: Etapas 3 (fase terminal/memoria, same_as_previous) y 4 (pre-identificación/catálogo).
